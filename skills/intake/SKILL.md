...
2. **Plan ALL panels up front, then fire the calls back-to-back — no deliberation between them.** The checklist is fixed input (every item is always asked — there is **no** decision, filtering, or branching to make between batches). So compute the *entire* plan in one pass — which items go in which panel, which panels go in which `AskUserQuestion` call (up to 4 panels per call) — **before firing call 1**. Then emit each call immediately after the previous returns: append the returned slugs to `selected` and fire the next call **with no extra thinking, no re-deriving the plan, no narration, no other tool calls** (no Read, no Bash). The ~2-minute stalls between batches are wasted xhigh-effort thinking on a step that has nothing to decide — don't think, just fire the next pre-planned call. (6 panels → 2 calls, 4-then-2.)
