...
- **No English description under Hebrew options.** In the §4 CHECKLIST panels, each option's `label` is the Hebrew checklist text and its `description` field is **left empty** — do NOT fill `description` with an English translation, gloss, or restatement of the Hebrew label. The only allowed description is the "If you're not sure, select it and we'll check together" subtitle on the first option of each panel (per §4.2). Anywhere else, leave description blank. Reason: bilingual labels clutter the panel and slow the user down — the Hebrew is the answer, English under it is noise.
